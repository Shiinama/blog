...
这篇文章主要介绍了海外 AI 项目的最佳实践技术选型，包括前后端框架选 Next，数据库选 Postgre 和 Orm 选 Prisma，登录鉴权库选 next-auth，AI API 选 Open Router 等，还提到了 UI 库、状态管理、验证、依赖一览表等内容，项目已上传至 github 并有预览地址和交流群。

关联问题: Next框架如何优化？ 数据库怎样高效运用？ 状态管理有何技巧？

AI智能总结首次生成速度较慢, 请耐心等待

## 引言

书接[上文](https://juejin.cn/post/7458279859510591539)，后面的段落开始就开始讲对于独立开发而言，`AI商业应用`在代码测的`最佳实践`。

那么我再次重复一下我对于最佳实践的论点：

> 对于每个人来说，看待一个项目的最佳实践是有不同视角的，有的人会想我希望我的网站能健全、能尽可能的承载更大的qps，有的人会觉得我的代码要利于维护要前后端分离......
> 
...
显然要验证商业其实根本都不用写代码，市面上如此之多的一键生成页面、AI生成应用的方式，最快的六十秒就能一个（填充布局，多布局），像wp这种市面一大堆成熟插件，用这些不是更好？

显然不行，因为要留下`自由度和可控性`，上述方式生成的`垃圾`在实际环境中非常难验证商业。最好的方式肯定还是自己写并加上`AI辅助`。

## 前后端不分离

首先这个背景是，我是独立开发，我要通过一个项目赚钱，我并不确定这个项目能否成功，我需要先用个东西验证这件事。
...
* **性能**: 在`ssr`的性能表现上`Nuxt`不如`Next`，在`csr`上肯定`react`不可能有`vue`的性能快好的，但问题是在于大量的`seo`和`scp`等一系列`c端核心指标`都依赖于`ssr`。而`csr`在现代浏览器的场景下，差的是那一点性能吗？
* **生态**: 在生态表现上`Next`拉`Nuxt`一条街。 就是大部分库 `React`系列有`Vue`一般会有， 但在一些库的深水区和issue数量这一点是不太经得起考验的， 再加上大多数海外各种服务的厂商对`React`系列的支持是多于`Vue`的。
* **易用度**: 在易用度上面一直是`V`系列的强项，狂拉`Next`一条街，它处理服务端组件和客户组件边界问题做得非常好。（在`两个框架的服务侧`方面，显然`next`的`server actions`属于是作弊级的开发体验， 但`nuxt`更强大的服务端配置以及也很出彩）

当然还有很多不同的对比角度，例如两个框架不同的缓存实现方法和控制方法等等等，但这都属于是在实践之后去慢慢理解的部分。

那综合考量，`Next`是最好的出海AI项目框架选择，`生态`永远是开发中最重要的一环。
...
_**支付隔离商：upgrade.chat**_

_**收款方式：PayPal、Stripe**_

## UI库和CSS

`ui库`你自己喜欢用啥就用啥，`radix/ui`、`shadcn`、`chakra`随便选一个都是你的`C`端好伙伴。根据各自喜欢就好，对于简单的我喜欢`radix/ui`。
...
这部分我也暂时没想好怎么做，因为量需要很大(大概一天1000篇3000字长文这种)，还不能被 `google`干成垃圾，这个度挺难把握的，人修的话太麻烦了。
...
项目传到`github`上了，有[预览地址](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2FShiinama%2Feasy-business-ai "https://github.com/Shiinama/easy-business-ai")。

也可以[加群](https://juejin.cn/user/400646714977431/pins)大家一起闲聊、学习、赚钱。