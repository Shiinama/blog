...
在React Fiber架构中，任务队列是实现可中断渲染和优先级调度的核心组件。本文主要讲`React源码`的`任务队列`、`可中断渲染`、`调度`、`同步模式`。

理解了这一整节，几乎就把 `react` 中最重要的东西和主线理解了，其余都是散散碎碎的小细节。
...
React的任务管理机制、任务中断恢复和车道协调共同构成了实现并发的核心框架。
...
7. **资源利用优化**：通过智能地分配CPU时间，React能够在保持UI响应性的同时，最大化利用可用的计算资源，这其实就是并发追求的东西。
...
React Fiber架构中的任务队列是实现可中断渲染和优先级调度的核心组件。通过深入分析其创建和调度过程，我们可以得出以下几点结论：
