...
在最简项目的前十章完成后，我们就可以很快的完成一个项目了。我以博客项目这个最通用的场景为例（永远的保底项目hhh），给大家过一遍这个流程。
...
在创建新项目成功后，`git clone https://github.com/Shiinama/blog-demo.git`，紧接着在文件夹下`pnpm install`。

### 创建Cloudflare全生态应用

创建D1数据库，`pnpm wrangler d1 create blog`，并复制到`wrangler.jsonc`。
...
将Jsonc配置的项目名和服务自引用名都改为`blog`。

剩余部分将在部署时自动绑定。

### 配置环境变量

`AUTH_SECRET`、`AUTH_GOOGLE_ID`、`AUTH_GOOGLE_SECRET`、`AUTH_RESEND_KEY`是可以直接复制过来，因为前面的课程已经实践过了，只需要在google cloud上面再次添加域名和回调地址。
