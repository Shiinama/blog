...
`"db:studio:local": "LOCAL_DB_PATH=$(find .wrangler/state/v3/d1/miniflare-D1DatabaseObject -type f -name '*.sqlite' -print -quit) drizzle-kit studio"`这段`package.json`里的脚本给大家解释一下，
LOCAL_DB_PATH环境变量的声明，是为了区别线上和测试环境+找到对应的本地`sqlite`文件，而`drizzle-kit studio`是drizzle启动可视化数据库窗的命令。
